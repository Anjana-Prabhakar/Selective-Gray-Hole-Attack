...
//@Anjana
INET_API long* getUpdateAfromIP();
INET_API long* getUpdateCfromIP();
...
public:
    ...
    int maliciousState;  // type of malicious activity
    double maliciousDropProbability;  // probability that a packet will be dropped (dependent on maliciousState)
    ...
    long numRxd[2]={0};
    long numRxdCurrent[2]={0};
    long numRxdPast[2]={0};

    // For stats
    static simsignal_t rcvdPkIPV4Signal;  // all packets received at the malicious code point
    static simsignal_t drpPkMaliciousSignal;  // all packets dropped maliciously

    static simsignal_t malCountSignal;
    static simsignal_t malStatusSignal;
    static simsignal_t detStatusSignal1;
    static simsignal_t detStatusSignal;
    static simsignal_t detStatusSignal3;


    static simsignal_t attkSignal;
    static simsignal_t possAttkSignal;

    static simsignal_t repShSignal;
    static simsignal_t repLoSignal;

    static simsignal_t fwdCurrSignal;
    static simsignal_t monCurrSignal;

    static simsignal_t fwdSignal;
    static simsignal_t rcvdSignal;

    static simsignal_t statSignal1;
    static simsignal_t statSignal2;

    static simsignal_t sentCountSignalA;
    static simsignal_t rxdCountSignalA;
    static simsignal_t sentCountSignalC;
    static simsignal_t rxdCountSignalC;

    simtime_t updateIntervalNetwork = 0.01 ;//10ms to 20ms 0.01
    simtime_t updateIntervalGateway = 0.2;//100ms to 200ms
    ...
    int flag;
    //@Anjana
    long numFwdDownstream=0;
    int numFwdPastA[2]={0};
    int numFwdPastC[2]={0};
    int numMonitorPast[2][2]={0};

    int numSentPast[2]={0};
    int numSentCurrent[2]={0};
    long numSentDownstream[2]={0};

    int channelCheck[2]={0};
    ...
    int feedbackScore=0;
    int countGatewayUpdate=0;

    int malActivityStatus=0;
    int malStatus[25]={0};
    int gatewayIndex=0;
    int gatewayStatusIndex=0;
    int gatewayIndexR1=0;
    int gatewayIndexR2=0;
    int gatewayIndexR3=0;
    int detectionMatrix1[25]={0};
    int detectionMatrix2[25]={0};
    int detectionMatrix3[25]={0};
    long numFwdTotal=0;
...
protected:
    ...
    //@Anjana
    int repFirst[2]={100,100};
    int repSecond=100;
    int repLong[2]={100,100};
    double repIntegrated[2]={0,0};
    int repShort[2]={0,0};
    int newR[2]={0,0};

    int updateMatrix[6][3]={0};

    //int numForwardedA = 0;
    //int numForwardedC = 0;

    int nodeIndex;
    double avgRep[5]={0,0,0,0,0};
    int attackRep=80;
    double deviation[4]={0,0,0};
    double deviationTotal;
    int dmax=16;//initial 18,12
    int statusCheck=5;//after every 5 gateway update, gateway decides whether a node is mal or not

    int countSGHA[3]={0,0,0};
    int countSFA[3]={0,0,0};
    int countPromo[3]={0,0,0};
    ...
    cMessage *networkUpdateTimer = nullptr;    // timer to send network updates
    cMessage *gatewayUpdateTimer = nullptr;    // timer to send gateway updates
...
protected:
    ...
    //@Anjana
    IPv4Datagram *updateGateway(IPv4Address src);
    void calculateReputationScoreShort(int nodeIndex);
    int *calculateReputationScoreLong(int nodeIndex);//confirm if void is sufficient
    void sendUpdateGateway(IPv4Address localIP);
    void sendUpdateNetwork(IPv4Address localIP);
    IPv4Datagram *updateNetwork(IPv4Address localIP);
    void detectAttack(int nodeIndex);
    IPv4Datagram * sendFeedback(int feedbackScore,int in_max,int nodeIndex,int numRxdCurr);
    int in_max,in_min=0;
